...
if __name__ == '__main__':
    input_dir = '/scratch/gobi2/tianxingli/raw_data/HD_dataset_MEMCNet'
    output_dir = '/scratch/gobi2/tianxingli/raw_data/HD_dataset_MEMCNet_frames'
    yuv_paths = glob.glob(os.path.join(input_dir, '**', '*.yuv'), recursive=True)
    for path in yuv_paths:
        print('Writing video ' + path + '...')
        dir_path = os.path.dirname(path)
        dirname = os.path.basename(dir_path)
        video_name = os.path.basename(path)[:-4]
        video_out_dir = os.path.join(output_dir, os.path.relpath(dir_path, start=input_dir), video_name)
        ...
        if res_c == '1':  # 1080p.
            ...
            raise ValueError('Third character of ' + dirname + ' was not as expected.')

        Reader = YUV_Read(path, h=height, w=width, toRGB=True)
        success = True
        count = 0
        while success:
            image, success = Reader.read()
            count += 1
            if success:
                img_path = os.path.join(video_out_dir, str(count).zfill(7) + '.png')
